...
def generate_attention_maps(N, K, snr, run, print_freq, n_head, n_layers, model, rate_profile, compositional):
    ...
    for iter in range(len(slf_attn_list)):
        for layer in range(slf_attn_list.shape[1]):
    ...
    for iter in range(len(slf_attn_list_no_noise)):  
        #print(iter)
        for layer in range(slf_attn_list_no_noise.shape[1]):
            for head in range(slf_attn_list_no_noise.shape[2]):
    ...
    for iter in range(len(slf_attn_list)):  
        #print(iter)
        for layer in range(slf_attn_list.shape[1]):
            for head in range(slf_attn_list.shape[2]):
...
if __name__ == "__main__":
    ...
    parser.add_argument('compos', type=bool, help='do you want compositional attention map or normal ones?')
    ...
    generate_attention_maps(N,K,snr,run,print_freq,n_head,n_layers,model,rate_profile,compos)
